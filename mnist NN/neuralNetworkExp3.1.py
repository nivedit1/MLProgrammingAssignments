...
if __name__ == '__main__':
    ...
    experiment_start_time_3dot1 = datetime.now()
    ...
    #Experiment 3.1 - Choosing a balanced training set after reducing by half

    print("Experiment 3.1 with 1/2 the training set")

    #Creating a reduced balanced training set - 1/2 the size
    newTrainingInputArrayRF2, newTrainingTargetArrayRF2 = generate_balanced(trainingInputImgArray, trainingTargetArray,2)
    ...
    for epoch in range(0, 50):
        ...
        for x in range(0, len(newTrainingInputArrayRF2)):
            inputArray = newTrainingInputArrayRF2[x].reshape((newTrainingInputArrayRF2[x].shape[0], 1))
            ...
            trainingTarget = create_target_array(newTrainingTargetArrayRF2[x][0])
            if (epoch > 0):
                deltaOutput, deltaHidden = calculateDelta(trainingHidden, outputData, trainingTarget, weights)
            ...
            if (trainingTarget[predictedData] == 0.9):
                trainingHits += 1
        trainingAccuracy = (trainingHits / len(newTrainingInputArrayRF2)) * 100
    ...
    print("Confusion Matrix for the test data when the training data was reduced by 1/2")
    print(confusionMatrix)

    # Plot training vs test accuracy for each experiment
    dual_plot_accuracy(trainingAccuracyArray, testAccuracyArray, 2)
    plt.close()

    print("Total Time taken for the experiment 3.1: ", datetime.now() - experiment_start_time_3dot1)